...
class Manual_invoice extends CI_Controller {
	...
	{	
		...
		$num=$this->db->get('manual_invoice')->result_array();
		...
		$this->load->view('manual_invoice',$data);
	...
	public function insert()
	{
		...
		$inv=date('Y-m-d',strtotime($_POST['invoicedate']));
...
// 			$this->db->insert('invoice_reports',$r);
...
// 			 }
	...
	{
		$data['bill']=$this->db->where('status',1)->order_by('id','desc')->limit(1)->get('manual_invoice')->result_array();
		...
		$result = implode('', $str);
		$data['fin']=$result;
		$this->load->view('manual_bill',$data);
	}
	public function view()
	{
		$data['invoice']=$this->manual_model->select();
		$this->load->view('header');
		$this->load->view('manual_reports',$data);
		$this->load->view('footer1');
	}
	public function staff_view()
	{
		$data['staff']=$this->manual_model->select();
		$this->load->view('header');
	...
	{
		$data['invoice']=$this->manual_model->search_invoice();
		$this->load->view('header');
		$this->load->view('manual_reports',$data);
		$this->load->view('footer1');
	}
	public function pdf()
	{
		...
		$todate=$_POST['todate'];
		$data['pdf']=$this->manual_model->search_invoice();
	...
	{
		$hk=$this->uri->segment(3);
		$data['invoice_edit']=$this->manual_model->head($hk);
		$this->load->view('header');
	...
	$vatamount='';
		...
		$count=count($codes);
		 ...
		 for($i = 0; $i< $count ; $i++)
			...
			$result=$this->manual_model->update_invoice($data,$id); 
			if($result==true)
			{
...
public function delete()
	{
		...
		$data=$this->db->where('id',$del)->delete('invoice_detail');
		if($data)
		{
		...
		{
			$this->session->set_flashdata('msg1','Invoice deleted  unsuccessfully');
			redirect('invoice/view');
		}
	}
	...
	public function reprint()
	{
		...
		$data['bill']=$this->db->where('status',1)->where('id',$id)->get('manual_invoice')->result_array();
		foreach($data['bill'] as $b)
		{
			$number= $b['grandtotal'];
		}
		...
		$result = implode('', $str);
		$data['fin']=$result;
		$this->load->view('manual_rebill',$data,$id);
